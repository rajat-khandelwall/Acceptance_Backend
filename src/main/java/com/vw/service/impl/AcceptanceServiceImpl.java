...
//			projectDetails.setGeneratedDate(LocalDate.now().toString());
    ...
    @Override
    public boolean sendPasswordResetOtp(String emailId) {
        // Check if email exists in the system
        SignupEmployee employee = loginRepository.findByEmailId(emailId); // Assuming `signupRepository` is defined
        if (employee == null) {
        ...
        // Save OTP to database or in-memory store (if applicable)
        employee.setOtp(String.valueOf(otp));
        loginRepository.save(employee);

        // Send OTP via email
        String subject = " AP Password Reset OTP";
        String body = "Your OTP for password reset is: " + otp + ". This OTP is valid for 10 minutes.";
		emailService.sendEmail(emailId, subject, body); // Assuming `emailService` handles email sending
